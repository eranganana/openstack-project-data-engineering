...
def fetch_data():
    ...
    for repo in selected_repos:
        ...
        dataset['data'].append([forks_count, size, forks_to_size_ratio, stargazers_to_forks_ratio])
...
tuner_gb = tune.Tuner(
    tune.with_parameters(train_model, data=dataset, model_type='gb'),
    param_space=search_space_gb,
    tune_config=tune.TuneConfig(metric="r2", mode="max")
)
result_gb = tuner_gb.fit()

best_gb = result_gb.get_best_result(metric="r2", mode="max")
print("Best GradientBoosting:", best_gb.config, "with R2:", best_gb.metrics["r2"])

# Save the best model
best_model = best_gb.metrics["model"]
save_model(best_model, "best_gradient_boosting_model")
